# EKS CLUSTER ROLE
resource "aws_iam_role" "EKSClusterRole-new" {
  name = "EKSClusterRole-new"
...
resource "aws_iam_role_policy_attachment" "AmazonEKSClusterPolicy" {
  ...
  role       = aws_iam_role.EKSClusterRole-new.name
}

# NODE GROUP ROLE
resource "aws_iam_role" "NodeGroupRole-new" {
  name = "EKSNodeGroupRole-new"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_iam_role_policy_attachment" "AmazonEKSWorkerNodePolicy" {
  ...
  role       = aws_iam_role.NodeGroupRole-new.name
}
